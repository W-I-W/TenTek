...
namespace Sisus.Init.ValueProviders
{
	...
	#endif
	{
		...
		bool IValueByTypeProvider.CanProvideValue<TValue>(Component client) => client && Find.typesToComponentTypes.ContainsKey(typeof(TValue));
		bool IValueByTypeProvider.CanProvideValue(Component client, Type valueType) => client && Find.typesToComponentTypes.ContainsKey(valueType);
