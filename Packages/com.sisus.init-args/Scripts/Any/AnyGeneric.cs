		...
		#endif
	{
		...
		GetValueAsync(Component client = null, Context context = Context.MainThread)
		{
			...
			return reference switch
			{
				null => value switch
				{
					null => await (client ? Service.GetForAsync<T>(client, context) : Service.GetAsync<T>()),
					Object unityObject when !unityObject => await (client ? Service.GetForAsync<T>(client, context) : Service.GetAsync<T>()),
					// Needed to support value providers in Any.Serialization.cs
