...
namespace Sisus.Init.EditorOnly.Internal
{
	...
	internal class ServiceDefinitionDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty serviceDefinitionProperty, GUIContent label)
		{
			...
			if((state.definingTypeProperty.GetValue() as _Type)?.Value is { } definingType
				&& !definingType.IsInstanceOfType(service)
				&& !ValueProviderUtility.IsValueProvider(service))
			{
				AnyPropertyDrawer.CurrentlyDrawnItemNullGuardResult = NullGuardResult.InvalidValueProviderState;
			}
