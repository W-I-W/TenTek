...
namespace Sisus.Init.EditorOnly.Internal
{
	...
	public sealed class InitializerGUI : IDisposable
	{
		...
		private static readonly GUIContent ValueProviderValueMissingLabel = new("Some value providers can't provide a value.",
		...
		public void OnInspectorGUI()
		{
			...
			try
			{
				...
				{
					nullGuardResult = NullGuardResult.ClientHidesAwake;
				}
				else
				{
				...
				if(!nullGuardDisabled && nullGuardResult is not NullGuardResult.Passed)
				{
					if(nullGuardResult is NullGuardResult.ValueMissing)
					{
						helpBoxMessage |= HelpBoxMessageType.TargetHasMissingDependencies;
					}
					else if(nullGuardResult is NullGuardResult.ValueProviderValueMissing or NullGuardResult.InvalidValueProviderState or NullGuardResult.ValueProviderException)
					{
						helpBoxMessage |= HelpBoxMessageType.ValueProviderValueMissing;
					}
				}
				...
				GUIContent GetNullGuardIconContent(ref Rect nullGuardIconRect, ref Color guiColor)
				{
					...
					if(nullGuardResult is NullGuardResult.InvalidValueProviderState)
					{
						nullGuardIcon = nullGuardFailedIcon;
					...
					if(nullGuardResult is NullGuardResult.ClientNotSupported)
					{
						nullGuardIcon = nullGuardFailedIcon;
					...
					if(nullGuardResult is NullGuardResult.TypeNotSupported)
					{
						nullGuardIcon = nullGuardFailedIcon;
					...
					if(nullGuardResult is NullGuardResult.ValueProviderException)
					{
						nullGuardIcon = nullGuardFailedIcon;
					...
					if(nullGuardResult is NullGuardResult.ClientException)
					{
						nullGuardIcon = nullGuardFailedIcon;
						if(string.IsNullOrEmpty(firstInitializerEditorOnly.NullGuardFailedMessage))
						{
					...
					if(nullGuardResult is NullGuardResult.ClientHidesAwake)
					{
						nullGuardIcon = nullGuardFailedIcon;
						nullGuardIcon.tooltip = GetTooltip(nullGuard, true, targetCanSelfInitializeWithoutInitializer) + "\n\nThe client defines an 'Awake()' method, causing the event function in the base class to be hidden. The 'OnAwake()' method should be overridden instead.";
						return nullGuardIcon;
					}
					
					if(nullGuardResult is NullGuardResult.ValueProviderValueMissing)
					{
						nullGuardIcon = nullGuardFailedIcon;
					...
					if(nullGuardResult is NullGuardResult.ValueProviderValueNullInEditMode)
					{
		...
		private static void DrawHelpBoxes(HelpBoxMessageType message, bool hasInitializerThatProvidesCustomInitArguments)
		{
			...
			else if(message.HasFlag(HelpBoxMessageType.ValueProviderValueMissing))
			{
				DrawHelpBox(MessageType.Warning, ValueProviderValueMissingLabel, HelpLinkLabel, HelpLinkURL);
			}
