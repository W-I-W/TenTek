		...
		private void OnAnyObjectChanged(ref ObjectChangeEventStream stream)
		{
			for(int s = stream.length - 1; s >= 0; s--)
			{
				...
				if(type is ObjectChangeKind.ChangeGameObjectStructure)
				{
					...
					if(EditorUtility.InstanceIDToObject(data.instanceId) is not { } changedGameObject)
					{
						continue;
					}
				...
				if(type is ObjectChangeKind.ChangeGameObjectOrComponentProperties)
				{
					...
					if(EditorUtility.InstanceIDToObject(data.instanceId) is not Component changedComponent)
					{
						continue;
					}
