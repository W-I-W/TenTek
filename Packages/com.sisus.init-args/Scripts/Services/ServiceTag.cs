...
namespace Sisus.Init.Internal
{
		...
		#endif
	{
		...
		internal static HashSet<ServiceTag> AllEditorOnly
		{
			get
			{
				foreach(var item in allEditorOnly)
				{
					if(!item)
					{
						allEditorOnly.Remove(item);
						return AllEditorOnly;
					}
				}
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		private void Deregister()
		{
			...
			if(service is not null && definingType.Value is { } definingTypeValue)
			{
